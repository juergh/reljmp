...
static void *(*reljmp_text_poke_smp)(void *, const void *, size_t) = NULL;
...
void reljmp_unregister(struct reljmp *jmp, int verbose)
{
	if (verbose) {
		printk("reljmp: unregister %s\n", jmp->from_symbol_name);
	}

	reljmp_lock();
	reljmp_text_poke_smp((void *)jmp->from_addr, jmp->from_code,
			     RELATIVEJUMP_SIZE);
	reljmp_unlock();

	if (verbose) {
		reljmp_info("reljmp: unpatched ", jmp->from_symbol_name,
			    (void *)jmp->from_addr);
	}
}
...
void reljmp_register(struct reljmp *jmp, int verbose)
{

	if (verbose) {
		printk("reljmp: register %s\n", jmp->from_symbol_name);
	}

	reljmp_lock();
	reljmp_text_poke_smp((void *)jmp->from_addr, jmp->jump_code,
			     RELATIVEJUMP_SIZE);
	reljmp_unlock();

	if (verbose) {
		reljmp_info("reljmp: patched ", jmp->from_symbol_name,
			    (void *)jmp->from_addr);
	}
}
